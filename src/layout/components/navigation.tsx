  ...
  return (
    <nav className="flex h-full flex-col w-full justify-between overflow-hidden">
      <section className='flex flex-col w-full gap-1 items-start p-4 overflow-y-auto relative overflow-x-hidden'>
        <Link to={PrivateRoutes.DASHBOARD} className={`${selectedMenu === PrivateRoutes.DASHBOARD ? 'text-light-action dark:text-dark-text-primary hover:bg-light-action-hover dark:bg-dark-border font-semibold' : 'text-light-text-secondary dark:text-dark-text-secondary'} h-10 flex items-center gap-3 rounded-md px-4 py-2 transition-all w-full hover:bg-light-action-hover hover:dark:bg-dark-border text-base font-normal`}>
                ...
                <Collapsible key={index} open={menuActive[item.label]} className='w-full'>
                  ...
                  >
                    <div
                      ...
                      ${isContract && selectedMenu.includes(item.path!) ? 'hover:bg-light-action-hover dark:bg-dark-border' : ''}
                      h-10 flex items-center justify-between gap-3 rounded-md pl-4 pr-2 py-2 transition-all w-full hover:bg-light-action-hover hover:dark:bg-dark-border text-base font-normal`}
                    >
                        ...
                        if (verifyPermission(child.permissions!)) {
                          return (
                            <Link
                              key={index}
                              to={child.path!}
                              className={`${selectedMenu === child.path ? 'bg-light-bg-secondary dark:bg-dark-border text-light-action dark:text-dark-text-primary font-semibold' : 'text-light-text-secondary dark:text-dark-text-secondary'} h-10 flex items-center gap-3 rounded-lg px-3 py-2 mt-1 transition-all hover:bg-light-action-hover hover:dark:bg-dark-border text-base font-normal w-full`}
                            >
                              {child.icon}
              ...
              if (verifyPermission(item.permissions!)) {
                return (
                  <Link
                    key={index}
                    to={item.path!}
                    className={`${selectedMenu.includes(item.path!) ? 'text-light-text-primary dark:text-dark-text-primary hover:bg-light-action dark:bg-dark-border font-semibold' : 'text-light-text-secondary dark:text-dark-text-secondary'} h-10 flex items-center gap-3 rounded-md px-4 py-2 mb-1 transition-all w-full hover:bg-light-action-hover hover:dark:bg-dark-border text-base font-normal`}
                  >
                    {item.icon}
