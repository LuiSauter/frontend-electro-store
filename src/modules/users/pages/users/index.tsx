      ...
      <Card x-chunk="dashboard-06-chunk-0" className='flex flex-col overflow-hidden w-full relative'>
            ...
            <Table>
              ...
              <TableBody>
                {isLoading
                  ...
                  : users?.filter((user: User) => [PERMISSION.ADMIN, PERMISSION.CASHIER].includes(user.role)).map((item: User) => (
                    <TableRow key={item.id}>
                      <TableCell>{item.name + ' ' + item.last_name}</TableCell>
                      <TableCell>{item.email}</TableCell>
                      <TableCell>{item.phone}</TableCell>
                      <TableCell>
                        <Badge variant={item.role === PERMISSION.ADMIN ? 'default' : 'secondary'}>
                          {item.role === PERMISSION.ADMIN ? 'Administrador' : 'Cajero'}
                        </Badge>
                        </TableCell>
                      <TableCell>
        ...
        <CardFooter className='w-full'>
          <Pagination
            ...
            currentItems={users?.filter((user: User) => [PERMISSION.ADMIN, PERMISSION.CASHIER].includes(user.role)).length ?? 0}
